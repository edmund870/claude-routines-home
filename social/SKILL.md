...
|----------|-------|-----------|-------|
| X / Twitter | sharp, direct, contrarian | 280-600 chars target, 600 cap | 1 post/run, thread every 4th run, media attached, no hashtags |
...
Algorithm tactics in `references/x-playbook.md`. Optimize for copy-link shares, replies,
quotes, and follow_author (the published weights, heaviest to lightest) — not bookmarks,
which carry no ranking weight at all. Stay in-niche, specific over generic, **attach a
chart/terminal-card image** (media = ~4.5x reach). Bias longer (280-600): the ranker's
dwell-time weight rewards posts that hold attention, so length earned by substance
outperforms short. Most runs post ONE standalone post built on the strongest single angle:
...
Every 4th run posts ONE connected thread (2-4 tweets, clean linear chain) — a dwell play on
one slot, not a reach multiplier (a thread collapses to its single best-scoring tweet per
viewer, and tweets 2+ are replies, ineligible for out-of-network retrieval). Tweet 1 must
stand alone. Winning format: "Why X:" lead → 2-3 specific numbers → skeptical kicker.
`$TICKER` format. No take-free newswire recitation; no hashtags; no engagement-bait CTAs.
